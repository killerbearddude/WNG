...
namespace
{
    ...
    void install_schema(wng::GraphSession& session, const wng::NodeDefinition& definition)
    {
        assert(session.schema().add_node_definition(definition) == wng::Result::Ok);
    }

    wng::NodeId create_session_schema_node(wng::GraphSession& session)
    {
        const wng::GraphSessionCommandResult result = session.create_schema_node(make_desc());
        assert(result.success());
        assert(result.command.record.node != wng::NodeId {});
        return result.command.record.node;
    }

    wng::PortId add_session_schema_port(
    ...
    wng::PortId add_session_graph_port(
        wng::GraphSession& session,
        wng::NodeId node,
        wng::PortKind kind,
        const char* name)
...
int main()
{
...

    {
        ...
        wng::GraphSession session;
        install_schema(session, make_definition());
...

    {
        ...
        wng::GraphSession session;
        install_schema(session, make_definition());
        const wng::NodeId source_node = create_session_schema_node(session);
        const wng::PortId output =
            add_session_schema_port(session, source_node, wng::PortKind::Output, "result");
...

    {
        ...
        install_schema(session, make_definition());
        const wng::NodeId source_node = create_session_schema_node(session);
        const wng::NodeId target_node = create_session_schema_node(session);
        const wng::PortId output =
            add_session_schema_port(session, source_node, wng::PortKind::Output, "result");
        const wng::PortId input =
            add_session_schema_port(session, target_node, wng::PortKind::Input, "a");
...

    {
        ...
        wng::GraphSession session;
        install_schema(session, definition);
        const wng::NodeId source_node = create_session_schema_node(session);
        const wng::NodeId target_node = create_session_schema_node(session);
        const wng::PortId output =
            add_session_graph_port(session, source_node, wng::PortKind::Output, "result");
        const wng::PortId input =
            add_session_graph_port(session, target_node, wng::PortKind::Input, "a");
